...
Notes (this repo’s current implementation):

- If `GOOGLE_PLACES_API_KEY` (or `GOOGLE_MAPS_API_KEY`) is set, `live_search` pulls hotels from **Google Places** via `agents/places_client.py` (names, ratings, photos, and Google Maps links).
- Otherwise, hotels are generated by the LLM as a fallback.
...
- Flight “enrichment” uses **Aviation Edge** routes (`AVIATION_EDGE_API_KEY`, with fallback to `AVIATIONSTACK_API_KEY`) to ground airlines/routes in real data.
- Booking links are Google Flights search URLs (clickable in the Streamlit UI).
